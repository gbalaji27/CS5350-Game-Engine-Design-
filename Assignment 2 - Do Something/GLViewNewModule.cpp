...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   float moveSpeed = 1.0f; // Adjust the movement speed as needed

   switch (key.keysym.sym)
   {
   case SDLK_UP:
       this->cam->moveRelative(Vector(moveSpeed, 0.0f, 0.0f));
       break;
   case SDLK_DOWN:
       this->cam->moveRelative(Vector(-moveSpeed, 0.0f, 0.0f));
       break;
   case SDLK_LEFT:
       this->cam->moveRelative(Vector(0.0f, moveSpeed, 0.0f));
       break;
   case SDLK_RIGHT:
       this->cam->moveRelative(Vector(0.0f, -moveSpeed, 0.0f));
       break;
   case SDLK_PAGEUP:
       this->cam->moveRelative(Vector(0.0f, 0.0f, moveSpeed));
       break;
   case SDLK_PAGEDOWN:
       this->cam->moveRelative(Vector(0.0f, 0.0f, -moveSpeed));
       break;
...
void Aftr::GLViewNewModule::loadMap()
{
...

   {
      ...
      int randomIndex = rand() % skyBoxImageNames.size();

      {
          // Create the SkyBox with randomly selected images
          WO* wo = WOSkyBox::New(skyBoxImageNames.at(randomIndex), this->getCameraPtrPtr());
          wo->setPosition(Vector(0, 0, 0));
          wo->setLabel("Sky Box");
          wo->renderOrderType = RENDER_ORDER_TYPE::roOPAQUE;
          worldLst->push_back(wo);
